...
exports.createData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      if (existing_data.entree.length === 0) {
        //on ajoute un objet avec un id et un name
        existing_data.entree.push({
          id: 1,
        ...
        const dataById = existing_data.entree.findLast((obj) => obj.id);
...
exports.deleteData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataById = existing_data.entree.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      ...
      if (!dataById) {
        ...
        existing_data.entree = existing_data.entree.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
