...
exports.getAllData = (request, response) => {
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      const existingData = JSON.parse(data);
      //ont assignent  dans une constante le tableau dessert
      const allData = existingData.dessert;
      //ont envoit une réponse avec le status 200 et le tableau dessert
...
exports.getDataById = (request, response) => {
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      //ont assignent dans une constante l'objet du tableau dessert dont l'id est égal à l'id de la requète
      const manipDataById = manipData.dessert.find(
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      if (!dataByName) {
        ...
        response.status(404).json({
          message: "Aucun dessert contenant ce nom dans les données",
          error: err,
        });
      } else {
