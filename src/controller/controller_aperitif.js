...
exports.createData = (request, response) => {
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      if (existingData.aperitif.length === 0) {
        //on ajoute un objet avec un id et un name
        existingData.aperitif.push({
          id: 1,
        ...
        const dataById = existingData.aperitif.findLast((obj) => obj.id);
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      if (!dataById) {
        ...
        existingData.aperitif = existingData.aperitif.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
