...
exports.createData = (request, response) => {
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      if (existingData.viande.length === 0) {
        //on ajoute un objet avec un id et un name
        existingData.viande.push({
          id: 1,
        ...
        const dataById = existingData.viande.findLast((obj) => obj.id);
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      if (!dataById) {
        ...
        existingData.viande = existingData.viande.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
