...
exports.createData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      //si le tableau viande est vide
      if (existing_data.viande.length === 0) {
        //on ajoute un objet avec un id et un name
        existing_data.viande.push({
          id: 1,
        ...
        const dataById = existing_data.viande.findLast((obj) => obj.id);
...
exports.getAllData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const allData = existingData.viande;
...
exports.getDataById = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      //ont assignent dans une constante l'objet du tableau viande dont l'id est égal à l'id de la requète
      const manipDataById = manipData.viande.find(
...
exports.getDataByName = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataByName = dataName.viande.find(
      ...
      if (!dataByName) {
        ...
        response.status(404).json({
          message: "Aucune viande contenant ce nom dans les données",
          error: err,
        });
      } else {
...
exports.updateData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataById = existingData.viande.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      if (!dataById) {
        response.status(404).json({
...
exports.deleteData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataById = existing_data.viande.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      ...
      if (!dataById) {
        ...
        existing_data.viande = existing_data.viande.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
