...
exports.createData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      //si le tableau poisson est vide
      if (existing_data.poisson.length === 0) {
        //on ajoute un objet avec un id et un name
        existing_data.poisson.push({
          id: 1,
        ...
        const dataById = existing_data.poisson.findLast((obj) => obj.id);
...
exports.getAllData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      //ont assignent  dans une constante le tableau poisson
      const allData = existingData.poisson;
      //ont envoit une réponse avec le status 200 et le tableau poisson
...
exports.getDataById = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      //ont assignent dans une constante l'objet du tableau poisson dont l'id est égal à l'id de la requète
      const manipDataById = manipData.poisson.find(
...
exports.getDataByName = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataByName = dataName.poisson.find(
      ...
      if (!dataByName) {
        ...
        response.status(404).json({
          message: "Aucun poisson contenant ce nom dans les données",
          error: err,
        });
      } else {
...
exports.updateData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataById = existingData.poisson.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      if (!dataById) {
        response.status(404).json({
...
exports.deleteData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataById = existing_data.poisson.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      ...
      if (!dataById) {
        ...
        existing_data.poisson = existing_data.poisson.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
