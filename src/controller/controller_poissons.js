...
exports.createData = (request, response) => {
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      if (existingData.poisson.length === 0) {
        //on ajoute un objet avec un id et un name
        existingData.poisson.push({
          id: 1,
        ...
        const dataById = existingData.poisson.findLast((obj) => obj.id);
  ...
  fs.readFile(cheminData, (err, data) => {
    ...
    if (err) {
      ...
      if (!dataById) {
        ...
        existingData.poisson = existingData.poisson.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
