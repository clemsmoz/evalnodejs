...
exports.createData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      //si le tableau digestif est vide
      if (existing_data.digestif.length === 0) {
        //on ajoute un objet avec un id et un name
        existing_data.digestif.push({
          id: 1,
        ...
        const dataById = existing_data.digestif.findLast((obj) => obj.id);
...
exports.getAllData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      //ont assignent  dans une constante le tableau digestif
      const allData = existingData.digestif;
      //ont envoit une réponse avec le status 200 et le tableau digestif
...
exports.getDataById = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      //ont assignent dans une constante l'objet du tableau digestif dont l'id est égal à l'id de la requète
      const manipDataById = manipData.digestif.find(
...
exports.getDataByName = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataByName = dataName.digestif.find(
      ...
      if (!dataByName) {
        ...
        response.status(404).json({
          message: "Aucun digestif contenant ce nom dans les données",
          error: err,
        });
      } else {
...
exports.updateData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataById = existingData.digestif.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      if (!dataById) {
        response.status(404).json({
...
exports.deleteData = (request, response) => {
  ...
  fs.readFile("model/data.json", (err, data) => {
    ...
    if (err) {
      ...
      const dataById = existing_data.digestif.find(
        (obj) => obj.id === parseInt(request.params.id)
      );
      ...
      if (!dataById) {
        ...
        existing_data.digestif = existing_data.digestif.filter(
          (obj) => obj.id != parseInt(request.params.id)
        );
